...
import { hamburgerToggle } from "src/redux/slices/LayoutSlice";

const NavbarComponent = () => {
  const dispatch = useDispatch();
  const isNavbarHamburgerOpen = useSelector(
    (state) => state.layout.navberHamburger
  );
  const navigation = useNavigate();
  const location = useLocation();

  const navberToggleOpen = () => {
    if (isNavbarHamburgerOpen) {
      dispatch(hamburgerToggle(false));
    } else {
      dispatch(hamburgerToggle(true));
    }
  };
  return (
    <>
      ...
      <nav className="fixed top-0 w-full z-20 shadow-lg lg:shadow-none bg-white">
            ...
            >
              ...
              <div className="userDropdown absolute right-0 mt-1 bg-gray-200 rounded">
                <li className="flex flex-col text-gray-700 w-48 shadow-lg">
                  <span
                    onClick={(e) => navigation("/user/orders")}
                  ...
                  <span
                    onClick={(e) => navigation("/user/profile")}
                    className="flex space-x-2 py-2 px-8 hover:bg-gray-400 cursor-pointer"
                  >
                    <span>
                      <svg
                  ...
                  <span
                    onClick={(e) => navigation("/wish-list")}
                    className="flex space-x-2 py-2 px-8 hover:bg-gray-400 cursor-pointer"
                  >
                    <span>
                      <svg
                  ...
                  <span
                    onClick={(e) => navigation("/user/setting")}
        ...
        >
          <div className="col-span-1 flex flex-col text-gray-600">
            ...
            <span
              className="font-medium text-lg tracking-widest hover:text-gray-800 hover:bg-gray-200 px-3 py-2 rounded-lg cursor-pointer"
              onClick={(e) => navigation("/blog")}
            >
              Blog
            </span>
            <span
              className="font-medium text-lg tracking-widest hover:text-gray-800 hover:bg-gray-200 px-3 py-2 rounded-lg cursor-pointer"
              onClick={(e) => navigation("/contact-us")}
            >
              Contact us
            </span>
          </div>
